...
void atk_ms53l0m_show_distance(uint8_t ready, uint16_t id, uint16_t *distance)
{
    if (ready && (atk_ms53l0m_modbus_get_data(id, distance) == ATK_MS53L0M_EOK))
    {
        OLED_ShowNum(1, 6, *distance, 4);
        OLED_ShowString(1, 11, "mm");
    }
    else
    {
